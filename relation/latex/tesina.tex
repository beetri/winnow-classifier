...
\subsection{Winnow}
Avendo scelto di utilizzare un algoritmo OnLine dobbiamo decidere quale algoritmo di base scegliere per poi testarlo ed adattarlo alle nostre esigenze.
...
Questo algoritmo ha delle qualità tra le quali spicca la capacita di non essere influenzato dalla presenza di molti attributi irrilevanti (``winnow \'e adatto in teoria per problemi con molti attributi irrilevanti'' \cite{Zhang}) e 
...
Per ora trascuriamo il primo blocco ed immaginiamo di che la domanda venga processata dal Feature Adder. La domanda viene convertita dal modulo di parsing in un vettore, più precisamente il vettore delle occorrenze.
...
L'aggiunta delle feature è un punto delicato di un sistema di classificazione e per tale motivo va analizzando ponderatamente e compatibilmente col dominio che viene trattato dal classificatore.
Il dominio di nostro interesse è composto da domande, le quali per loro propria natura sono ``compattate'' in media in una certa finestra di lunghezza comunque molto piccola.
L'obiettivo del preprocessing è arricchire le informazioni così da averne poi un beneficio diretto sul processo di classificazione immediatamente successivo.

Le nostre feature hanno lo scopo di:
\begin{enumerate}
	\item Tener conto in qualche modo dell'ordine delle parole;
	\item Tener conto del valore semantico delle parole.
\end{enumerate}
Per soddisfare questi obiettivi sono state aggiunti nuovi elementi creati dalla giustapposizione delle parole presenti nelle domande.
...
Con la tecnica sopra descritta sono state agiunte le segueni feature:
\begin{enumerate}
	\item Relazioni tra le parole sia a coppie che a triple;
	\item Parole ``stemmate'' (e non), nel dettaglio tramite lo ``Snowball Stemmer'' ed un altro rudimentale sviluppato da noi;
	\item Relazione tra la prima parola della domanda e l'ultima;
	\item Utilizzo di WordNet per la derivazione del tipo di parola. % Referenza su wordnet %
\end{enumerate}
